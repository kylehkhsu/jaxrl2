...
config_flags.DEFINE_config_file(
    'config',
    './configs/offline_pixels_config.py:bc',
...
def main(_):
    ...
    replay_buffer = env.q_learning_dataset(include_pixels = True)
    replay_buffer.seed(FLAGS.seed)
    replay_buffer_iterator = replay_buffer.get_iterator(FLAGS.batch_size)
    print('Replay buffer loaded')

    data = next(replay_buffer_iterator)
    print('Keys:', data.keys())
    print('Shape:', data['observations']['pixels'].shape)
    print('Start training')
    print("FLAGS.tqdm:", FLAGS.tqdm)
    for i in tqdm.tqdm(range(1, FLAGS.max_steps),
        ...
        if i % FLAGS.log_interval == 0:
            ...
            if not FLAGS.tqdm:
                print(f"[BC seed {FLAGS.seed}] {i}/{FLAGS.max_steps} steps")
