...
class DeepMindControl:
    def __init__(self, name, size=(64, 64), camera=None):
        ...
        self._observation_space = gym.spaces.Dict({"pixels":gym.spaces.Box(0, 255, self._size + (3,), dtype=np.uint8)})
        spec = self._env.action_spec()
        self._action_space = gym.spaces.Box(spec.minimum, spec.maximum, dtype=np.float32)

    def seed(self, seed):
        print(f"Setting currently not implemented.")
    ...
    def observation_space(self):
        ...
        # spaces['pixels'] = gym.spaces.Box(0, 255, self._size + (3,), dtype=np.uint8)
